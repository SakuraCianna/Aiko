...
Planner / Retriever / Executor 拆分, 但不单独手写复杂 `StateGraph`。状态边界由
runtime 编排, 具体职责分散到 `src/main/agent/retriever`,
`src/main/agent/planner`, `src/main/agent/executor`, `src/main/agent/tools`
和 `src/main/agent/trace`。后续如果需要多节点工作流, 再在这个 runtime 内引入显式
LangGraph graph。
...
2. `AikoRetriever` 召回记忆, 整理附件, 语音结果和工具提示。
3. `AikoPlanner` 先处理确定性本地计划。
4. `AikoExecutor` 将结构化计划转换为待确认动作或阻断结果。
5. 无确定性计划时, runtime 调用 LangChain Agent。
6. LangChain tools 只生成待确认动作, 不执行本地系统操作。
7. 回复后进入静默记忆候选抽取。
8. 普通聊天回复会写入短期上下文窗口, 下一轮模型调用会带上这个窗口。
...
- 需要多步状态的能力：在 `AikoAgentRuntime` 内引入显式 LangGraph graph 或 middleware。
