...
和 `src/main/agent/trace`。LangGraph checkpoint 和 human-in-the-loop interrupt
...
- `interrupt`: 可恢复审批模式, workflow 会用 LangGraph `interrupt` 暂停 pending action,
  ...
  当前 runtime 默认使用 `interrupt`, `passive` 只作为兼容和测试模式保留。

## Subagent Boundary

当前已经开始把单一 Agent 生命周期拆成可替换的子 Agent, 但主入口仍然保持在
`AikoAgentRuntime`。第一批子 Agent 只拆稳定边界:

- `src/main/agent/subagents/researchAgent.ts`: 负责 Tavily MCP 网页检索和 Open-Meteo 等固定输入输出实时知识 provider。它只返回 grounding 上下文, 不直接回复用户, 也不执行本地动作。
- `src/main/agent/subagents/memoryAgent.ts`: 负责长期记忆召回和回复后的静默记忆候选写入。它统一处理召回失败降级, 记忆候选去重和状态分类。

`AikoRetriever` 现在只负责把子 Agent 的结果格式化进模型上下文, 不再直接调用每一个外部 provider。
`AikoAgentRuntime` 现在通过 `MemoryAgent.rememberExchange()` 写入长期记忆, 不再内联记忆去重和分类逻辑。
后续拆 BrowserAgent, FileAgent, MCPAgent 时优先沿用这个模式: 子 Agent 产出结构化结果或待确认动作, runtime/graph 负责生命周期和权限边界。
...
- 麦克风录音先进入 `SpeechUnderstandingProvider`, 当前默认 provider 是本地 faster-whisper HTTP adapter。
- 回复语音通过 renderer 调用 `voice:synthesize`, 当前默认 provider 是本地 CosyVoice HTTP adapter, 不可用时回退 Web Speech。
- 设置面板通过 `voice:status` 检查 ASR/TTS 本地服务的 `/health` endpoint, 只展示状态, 不主动启动服务。
...
- ResearchAgent 已经接管 Tavily MCP 检索和 Open-Meteo 天气 grounding。
- MemoryAgent 已经接管长期记忆召回, 静默记忆候选去重和保存。
- 长期记忆已接入 `sqlite-vec` 的 `vec0` 向量索引, `memory_vectors` 稀疏向量表作为扩展不可用时的降级路径。
- Agent Trace 和 Action Journal 已接入 SQLite 审计仓库, 用于跨运行期排查请求生命周期和本地动作结果。
- LangGraph checkpoint 已接入 node:sqlite 持久化, interrupt 审批在应用重启后可以用同一 thread id 恢复。
- Windows 高风险能力已经进入策略矩阵, `read_file`, `write_file`, `window_control`, `run_shell_command` 默认全部拒绝。
...
3. `AikoRetriever` 调用 MemoryAgent 和 ResearchAgent, 再整理附件, 语音结果和工具提示。
4. `AikoPlanner` 先处理确定性本地计划。
5. `AikoExecutor` 将结构化计划转换为待确认动作或阻断结果。
6. Workflow review 节点把待确认动作转换成人审 payload, 默认启用 interrupt + resume 审批链路。
7. 无确定性计划时, runtime 调用 LangChain Agent。
8. LangChain tools 只生成待确认动作, 不执行本地系统操作。
9. 回复后交给 MemoryAgent 静默抽取并保存记忆候选。
...
- faster-whisper 和 CosyVoice 的本地服务安装脚本与健康检查。
- zero-shot voice cloning。
- Model Adapter。
- 更细的 Memory Policy, 以及可选真实 embedding provider。
- 更高风险的 Windows 能力需要单独设计沙箱, 参数校验, 审计和回滚策略后再开放。
...
- 需要外部资料或固定 API grounding 的能力：优先扩展 `ResearchAgent`, 再由 Retriever 注入不可信上下文。
- 需要长期记忆读写策略的能力：优先扩展 `MemoryAgent`, 不要把记忆整理逻辑写回 runtime。
- 长期记忆：通过 Retriever 注入召回结果；回复后抽取候选并写入 SQLite。
- 多模态：统一由 Retriever 组装进 LangChain message content。
- 音频输入：先走 `SpeechUnderstandingProvider` 得到 transcript，再交给 Agent。当前使用 faster-whisper adapter。
- 语音输出：先走 `SpeechSynthesisProvider` 得到音频 data URL，再由 renderer 播放。当前使用 CosyVoice adapter, 失败时回退 Web Speech。
...
- ResearchAgent 和 MemoryAgent 存在, Retriever/Runtime 通过子 Agent 边界接入检索和记忆
