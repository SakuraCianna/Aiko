...
- 麦克风录音先封装为 WAV 附件, 再进入 `SpeechUnderstandingProvider`, 当前默认 provider 是腾讯云一句话识别 adapter。
...
- Renderer 已提供独立动作审计面板, 用于查看本地动作的规划, 审批, 执行结果和回滚提示。
...
- 腾讯云 ASR/TTS 实机调用验证和音色调优。
- zero-shot voice cloning。
- Model Adapter。
- 更细的 Memory Policy, 以及可选真实 embedding provider。
- 更高风险的 Windows 能力需要继续完善沙箱, 参数校验和自动回滚策略后再开放。
...
- 音频输入：先走 `SpeechUnderstandingProvider` 得到 transcript，再交给 Agent。当前使用腾讯云 ASR adapter。
- 语音输出：先走 `SpeechSynthesisProvider` 得到音频 data URL，再由 renderer 播放。当前使用腾讯云 TTS adapter, 失败时回退 Web Speech。
