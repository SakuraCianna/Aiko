...
- 简单高频命令先走确定性规则：打开应用、打开 URL、网页搜索、分钟/小时提醒、绝对时间提醒、目录列举、文件读取和截屏查看桌面。
...
- Tool Registry 已统一记录 `open_application`, `open_url`, `web_search`,
  `create_reminder`, `recall_memory`, `list_reminders`, 文件系统动作, 受控 Shell, 截屏, 窗口控制和键鼠输入的工具元信息。
...
- Windows 高风险和 critical 能力已经进入策略矩阵。文件系统, 受控 Shell, 截屏, 窗口控制和键鼠输入只能生成待确认动作, 不能被模型直接执行。
