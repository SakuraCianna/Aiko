...
const IDLE_AMBIENT_MOTIONS: CharacterMotion[] = ["idleShift", "settle", "curious", "wake"];
...
// 选择低频待机小动作, 让桌宠在没有任务时也有轻微存在感.
export function selectIdleAmbientMotion(seed: number): CharacterMotion {
  return IDLE_AMBIENT_MOTIONS[Math.abs(Math.floor(seed)) % IDLE_AMBIENT_MOTIONS.length] ?? "idleShift";
}

// 等待用户确认越久, 动作越从轻提示转向专注等待.
export function selectWaitingCue(waitingSeconds: number): CharacterCue {
  return {
    behavior: "waiting",
    motion: waitingSeconds >= 30 ? "focus" : "wait"
  };
}

// 区分用户显式中止和新请求替换, 避免所有打断都表现成同一种失败.
export function selectInterruptionCue(kind: "cancelled" | "replaced"): CharacterCue {
