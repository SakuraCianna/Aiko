...
当前项目重点是先把 Agent 基础, 权限边界, 记忆系统, 桌宠 UI 和本地语音链路跑稳。ASR 已按 faster-whisper provider 预留, TTS 已按 CosyVoice provider 接入播放链路, 但需要你本地启动对应服务。
...
- 图片附件入口。
- 默认麦克风录音入口。
- faster-whisper ASR provider, 用于把录音附件转成 transcript。
- CosyVoice TTS provider, 用于把 Aiko 回复合成为高质量本地语音。
...
- 图片可以进入多模态输入结构。
- 录音会先进入 faster-whisper ASR provider, 前提是 `AIKO_ASR_ENABLED=true` 且本地 ASR 服务已经启动。
...
1. 启动本地 faster-whisper 服务, 验证录音附件转 transcript。
2. 启动本地 CosyVoice 服务, 验证回复播放和打断。
3. 给 CosyVoice 增加 Aiko 专属音色配置。
...
请先确认 `.env` 中 `AIKO_ASR_ENABLED=true`, 并且本地 faster-whisper HTTP 服务正在 `AIKO_ASR_BASE_URL` 监听。没有 ASR 服务时, Aiko 会明确知道语音理解未配置, 不会编造录音内容。
