...
- 受控高风险动作: 明确的目录列举, 文件读取, 文件写入, 删除到 Aiko trash, 从 Aiko trash 恢复和 PowerShell 命令请求可以进入待确认动作。
- 文件安全回滚: 覆盖写入前会备份旧文件, 删除文件会写入恢复元数据, 恢复动作会检查目标路径是否已存在。
- Shell 安全边界: PowerShell 只允许单条只读 allowlist cmdlet, 会拒绝管道, 重定向, 分号, 嵌套 shell 调用和敏感文件目标。
...
- 动作审计面板, 用于查看本地动作的规划, 审批, 执行结果和回滚提示。

暂未实现:

- zero-shot voice cloning。
- 窗口控制, 截图分析, 键鼠自动化。
- 高风险 Windows 自动化的完整撤销后台。
- 完整低延迟双工实时语音, 包括流式 ASR, 播放队列, 中止播放和口型同步闭环。
- 多角色系统。
...
- Planner 先尝试用确定性逻辑识别命令, 比如打开应用, 创建提醒, 目录列举, 文件读取和受控 PowerShell 请求。
...
- `medium`: 读取剪贴板, 截图, 列出用户明确指定的目录。
- `high`: 读取文件, 写文件, 删除文件, 执行命令。
- `critical`: 批量文件操作, 系统设置修改, 高权限自动化。

当前版本已经开放一部分受控高风险能力。它们必须进入待确认和审计链路, 不会被永久授权, Shell 和文件能力仍受本地路径, 只读命令 allowlist, 命令语法校验和敏感文件阻断限制。文件覆盖会先备份旧内容, 删除会移动到 Aiko trash 并记录恢复元数据, 从 trash 恢复时如果目标路径已有新文件会停止。
...
- 图片可以进入多模态输入结构。
- 录音会先封装成 WAV 附件, 停止录音后自动提交, 再进入腾讯云 ASR provider, 前提是 `AIKO_ASR_ENABLED=true` 且腾讯云密钥已配置。
...
- 当前还不是完整低延迟双工实时语音, 后续需要补流式 ASR, 播放队列, 中止播放和口型同步。
...
- 待确认动作有过期时间。
- 高风险动作每次都需要确认, 并且会进入动作审计日志。
- 覆盖写入文件前会备份旧版本, 删除文件只会移动到 Aiko trash, 从 trash 恢复需要再次确认。
- Shell 命令只允许只读 allowlist 内的单条 PowerShell cmdlet, 不允许链式脚本, 管道, 重定向或嵌套 shell。
- 模型输出不能直接当作命令执行。
...
请先确认 `.env` 中 `AIKO_ASR_ENABLED=true`, 并且 `TENCENTCLOUD_SECRET_ID` 与 `TENCENTCLOUD_SECRET_KEY` 已配置。点击麦克风开始录音, 再次点击停止后会自动提交语音附件。没有 ASR 密钥时, Aiko 会明确知道语音理解未配置, 不会编造录音内容。

### 为什么有些 Windows 操作不能做?

这是刻意设计。当前阶段已经打通低风险能力, 以及一部分受控高风险能力, 例如目录列举, 文件读取, 文件写入, 删除到 Aiko trash, 从 Aiko trash 恢复和受控 PowerShell 命令。高风险动作每次都要确认, 不能被永久授权, 并且会进入动作审计日志。PowerShell 目前只允许只读 allowlist 命令。窗口控制, 截图分析, 键鼠自动化和系统设置修改仍未开放。
