...
- 受控高风险动作: 明确的目录列举, 文件读取, 文件写入, 删除到 Aiko trash 和 PowerShell 命令请求可以进入待确认动作。
...
当前版本已经开放一部分受控高风险能力。它们必须进入待确认和审计链路, 不会被永久授权, Shell 和文件能力仍受本地路径, 命令校验和敏感文件阻断限制。
...
这是刻意设计。当前阶段已经打通低风险能力, 以及一部分受控高风险能力, 例如目录列举, 文件读取, 文件写入, 删除到 Aiko trash 和受控 PowerShell 命令。高风险动作每次都要确认, 不能被永久授权, 并且会进入动作审计日志。窗口控制, 截图分析, 键鼠自动化和系统设置修改仍未开放。
